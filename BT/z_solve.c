    ...
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (k = 1; k <= ksize-1; k++) {
		...
		for(z = a; z <= b; z++){
			...
			fjac[k + z][0][1] = - ( u[k + z][j][i][1]*u[k + z][j][i][3] ) * tmp2;
			fjac[k + z][1][1] = u[k + z][j][i][3] * tmp1;
			fjac[k + z][2][1] = 0.0;
			fjac[k + z][3][1] = u[k + z][j][i][1] * tmp1;
			fjac[k + z][4][1] = 0.0;

			fjac[k + z][0][2] = - ( u[k + z][j][i][2]*u[k + z][j][i][3] ) * tmp2;
			fjac[k + z][1][2] = 0.0;
			fjac[k + z][2][2] = u[k + z][j][i][3] * tmp1;
			fjac[k + z][3][2] = u[k + z][j][i][2] * tmp1;
			fjac[k + z][4][2] = 0.0;

			fjac[k + z][0][3] = - (u[k + z][j][i][3]*u[k + z][j][i][3] * tmp2 ) 
			  + c2 * qs[k + z][j][i];
			fjac[k + z][1][3] = - c2 *  u[k + z][j][i][1] * tmp1;
			fjac[k + z][2][3] = - c2 *  u[k + z][j][i][2] * tmp1;
			fjac[k + z][3][3] = ( 2.0 - c2 ) *  u[k + z][j][i][3] * tmp1;
			fjac[k + z][4][3] = c2;

			fjac[k + z][0][4] = ( c2 * 2.0 * square[k + z][j][i] - c1 * u[k + z][j][i][4] )
			  * u[k + z][j][i][3] * tmp2;
			fjac[k + z][1][4] = - c2 * ( u[k + z][j][i][1]*u[k + z][j][i][3] ) * tmp2;
			fjac[k + z][2][4] = - c2 * ( u[k + z][j][i][2]*u[k + z][j][i][3] ) * tmp2;
			fjac[k + z][3][4] = c1 * ( u[k + z][j][i][4] * tmp1 )
			  - c2 * ( qs[k + z][j][i] + u[k + z][j][i][3]*u[k + z][j][i][3] * tmp2 );
			fjac[k + z][4][4] = c1 * u[k + z][j][i][3] * tmp1;
			...
			njac[k + z][0][1] = - c3c4 * tmp2 * u[k + z][j][i][1];
			njac[k + z][1][1] =   c3c4 * tmp1;
			njac[k + z][2][1] =   0.0;
			njac[k + z][3][1] =   0.0;
			njac[k + z][4][1] =   0.0;

			njac[k + z][0][2] = - c3c4 * tmp2 * u[k + z][j][i][2];
			njac[k + z][1][2] =   0.0;
			njac[k + z][2][2] =   c3c4 * tmp1;
			njac[k + z][3][2] =   0.0;
			njac[k + z][4][2] =   0.0;

			njac[k + z][0][3] = - con43 * c3c4 * tmp2 * u[k + z][j][i][3];
			njac[k + z][1][3] =   0.0;
			njac[k + z][2][3] =   0.0;
			...
			njac[k + z][0][4] = - (  c3c4
				- c1345 ) * tmp3 * (u[k + z][j][i][1]*u[k + z][j][i][1])
			  - ( c3c4 - c1345 ) * tmp3 * (u[k + z][j][i][2]*u[k + z][j][i][2])
			  - ( con43 * c3c4
				  - c1345 ) * tmp3 * (u[k + z][j][i][3]*u[k + z][j][i][3])
			  - c1345 * tmp2 * u[k + z][j][i][4];

			njac[k + z][1][4] = (  c3c4 - c1345 ) * tmp2 * u[k + z][j][i][1];
			njac[k + z][2][4] = (  c3c4 - c1345 ) * tmp2 * u[k + z][j][i][2];
			njac[k + z][3][4] = ( con43 * c3c4
				- c1345 ) * tmp2 * u[k + z][j][i][3];
