  ...
  for (k = 1; k <= grid_points[2]-2; k++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (m = 0; m < 5; m++) {
          (dvmh_get_own_element_addr_C(u, 4, DVM0C(k), DVM0C(j), DVM0C(i), DVM0C(m)) ? ((*(double *)dvmh_get_own_element_addr_C(u, 4, DVM0C(k), DVM0C(j), DVM0C(i), DVM0C(m))) = (*(double *)dvmh_get_element_addr_C(u, 4, DVM0C(k), DVM0C(j), DVM0C(i), DVM0C(m))) + rhs[k][j][i][m]) : ((void)0));
        }
      }
    }
  }
